...
static enum mailbox_copy_type antispam_classify_copy(
		enum mailbox_class src,
...
static int antispam_save_finish(struct mail_save_context *ctx)
{
	...
	return copy_type == MCT_IGNORE ? 0 :
		asu->backend->handle_mail(t, ast->data, ctx->dest_mail,
			copy_type == MCT_SPAM);
}
...
static void
antispam_transaction_rollback(struct mailbox_transaction_context *t)
{
	struct antispam_mailbox *asmb = STORAGE_CONTEXT(t->box);
	struct antispam_user *asu = USER_CONTEXT(t->box->storage->user);
	struct antispam_transaction *ast = TRANSACTION_CONTEXT(t);
